...
Procedure Puente is
	...
	TASK BODY Acceso IS
		peso: integer := 0; //cant. en el puente
	BEGIN
		LOOP
			SELECT 
				WHEN (peso < 5) => 
...
Procedure Banco is
	...
	BEGIN
        SELECT		
            Empleado.Pedido(“Pago”, comprobante); //espera atencion, es atendido
        ELSE
            NULL; //si el empleado no realiza el accept de pedir inmediatamente ejecuta el null
        END SELECT;
	END Cliente;
...
Procedure Sistema is
	...
	TASK BODY Central IS
		fin: boolean;
	BEGIN
		ACCEPT TomarSenialP1(senial: IN text); //los ; son importantes porque hay que marcar que no tienen cuerpo!
		LOOP
			SELECT
...
### Respuestas:
//este chichi nos lo empezó a explicar pero nos teníamos que ir, marcó que  se generaba busy waiting que si bien "está permitido" en MD siempre es aconsejable evitarlo, **esto debería estar en una entry call, también dijo que convenía agregar una task recepcionista y dijo algo tmb anoté: secuencial hacer escritorio pedirnota, medico atenderNota, sabrá D10s qué quise poner (?
```
